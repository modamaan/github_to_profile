import { db } from './db';
import { customUrl } from '@/lib/db/schema';
...
export async function isCustomSlugAvailable(slug: string): Promise<boolean> {
  ...
  try {
    const result = await db
      .select()
      .from(customUrl)
      .where(eq(customUrl.customSlug, slug.toLowerCase()))
      .limit(1);
...
export async function getGithubUsernameByCustomSlug(slug: string): Promise<string | null> {
  try {
    ...
    return result[0]?.githubUsername || null;
  } catch (error) {
...
export async function createCustomUrl(customSlug: string, githubUsername: string): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    const result = await db
      .select()
      .from(customUrl)
      .where(eq(customUrl.customSlug, normalizedSlug))
      .limit(1);
    ...
    await db.insert(customUrl).values({
      id: nanoid(),
